...
void Newton_For(vector<vector<long double>> &grid, long double p, int n)
{
    ...
    for(int i=1; i<n; i++) y += ((long double)get_p_for(i) / Fact(i) ) * grid[0][i];
    cout << y << nn;
}
...
void Newton_Bac(vector<vector<long double>> &grid, long double p, int n)
{
    ...
    for(int i=1; i<n; i++) y += ((long double)get_p_bac(i) / Fact(i) ) * grid[n-1-i][i];
    cout << y << nn;
}
...
{
    int midd = n / 2;
    long double factor, term, y = grid[midd][0];
    if(comm == 1) //forward
    {
